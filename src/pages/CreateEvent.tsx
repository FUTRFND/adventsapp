    ...
    <div className="pb-24 min-h-screen">
      <div className="sticky top-0 bg-background z-10 px-5 pt-14 pb-4">
        <div className="flex items-center gap-4 mb-4">
          ...
          <h1 className="text-xl font-display font-bold text-foreground flex-1 text-center pr-6">Create Event</h1>
        </div>
      </div>

      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="px-5 space-y-5">
        <div>
          <Label className="text-xs text-muted-foreground mb-1.5 block">Event Name</Label>
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="My Amazing Event" className="h-12 bg-secondary border-0 text-foreground font-medium" />
        </div>

        <div>
          <Label className="text-xs text-muted-foreground mb-1.5 block">Type</Label>
          <Select value={type} onValueChange={setType}>
            <SelectTrigger className="h-12 bg-secondary border-0"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input type="date" value={dateStart} onChange={(e) => setDateStart(e.target.value)} className="h-12 bg-secondary border-0 text-foreground" />
          </div>
          <div>
            <Label className="text-xs text-muted-foreground mb-1.5 block">End Date</Label>
            <Input type="date" value={dateEnd} onChange={(e) => setDateEnd(e.target.value)} className="h-12 bg-secondary border-0 text-foreground" />
          </div>
        </div>
        ...
        <Button className="w-full py-6 text-base font-semibold mt-4" onClick={handleCreate} disabled={saving}>
          {saving ? "Creating..." : "Create & Plan Event"}
        </Button>
      </motion.div>
    </div>
  );
